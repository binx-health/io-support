...
#define USB_OffsetOfDBT (((uint32_t)__segment_size("USB_PACKET_MEMORY")/2) - (sizeof(USB_BuffDeskTbl_t) * (MaxIndOfRealizeEp+1)) & ~7)

#define __Offset2Addr_PackMem(Offset) ((uint32_t)__segment_begin("USB_PACKET_MEMORY") + ((Offset<<1) & ~3) + (Offset&1))
...
typedef uint32_t UsbDefStatus_t;

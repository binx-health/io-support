...
__root __no_init uint32_t USB_PacketMemory[256];
 ...
 * Parameters: volatile pInt32U pReg, uint32_t Data
 *
 * Return: none
 *
 ...
 *
 *************************************************************************/
void EpCtrlSet_EA(volatile pInt32U pReg, uint32_t Data)
{
  Data &= 0xF;
  Data |= *pReg & 0x0700 ;
...
uint32_t State;
  ...
  if(EpCtrlGet_STAT_TX(pReg) != (uint32_t)Data)
  {
    State = Data;
    State <<= 4;
    State ^= *pReg;
 ...
 *************************************************************************/
inline
uint32_t EpCtrlGet_STAT_TX (volatile pInt32U pReg)
{
uint32_t Data = *pReg;
 ...
 *
 *************************************************************************/
void EpCtrlSet_DTOG_TX(volatile pInt32U pReg, uint32_t Data)
{
 ...
 *************************************************************************/
inline
uint32_t EpCtrlGet_DTOG_TX(volatile pInt32U pReg)
{
 ...
 *************************************************************************/
inline
uint32_t EpCtrlGet_CTR_TX(volatile pInt32U pReg)
{
 ...
 *
 *************************************************************************/
void EpCtrlSet_EP_KIND (volatile pInt32U pReg, uint32_t Data)
{
  Data  = Data ? (1UL << 8):0;
 ...
 *************************************************************************/
inline
uint32_t EpCtrlGet_EP_KIND(volatile pInt32U pReg)
{
 ...
 *************************************************************************/
inline
uint32_t EpCtrlGet_EP_TYPE(volatile pInt32U pReg)
{
uint32_t Data = *pReg;
  Data &= (3UL<<9);
  return(Data >> 9);
 ...
 *************************************************************************/
inline
uint32_t EpCtrlGet_SETUP(volatile pInt32U pReg)
{
uint32_t Data = *pReg & (0x1UL << 11);
  return(Data?1:0);
}
...
uint32_t State;
  ...
  // operations from VALID to NAK and result of
  // set operation will be invalid!
  if(EpCtrlGet_STAT_RX(pReg) != (uint32_t)Data)
  {
    State = Data;
    State <<= 12;
    State  ^= *pReg;
 ...
 *************************************************************************/
inline
uint32_t EpCtrlGet_STAT_RX(volatile pInt32U pReg)
{
uint32_t Data = *pReg &( 0x3UL << 12);
  return(Data >> 12);
}
 ...
 *
 *************************************************************************/
void EpCtrlSet_DTOG_RX(volatile pInt32U pReg, uint32_t Data)
{
 ...
 *************************************************************************/
inline
uint32_t EpCtrlGet_DTOG_RX(volatile pInt32U pReg)
{
 ...
 *************************************************************************/
inline
uint32_t EpCtrlGet_CTR_RX(volatile pInt32U pReg)
{
 ...
 * Parameters: pInt16U pOffset,uint32_t PacketSize,
...
//uint32_t MaxPacketSize = *pPacketSize+4;  // for CRC
uint32_t MaxPacketSize = *pPacketSize;
  ...
  *pOffset = 0;
  ...
  if((*pOffset + MaxPacketSize) >=
     (((uint32_t)__segment_size("USB_PACKET_MEMORY") -
...
#else
  GPIO_WriteBit(GPIOB,GPIO_Pin_1,((Conn)?Bit_SET:Bit_RESET));
#endif  
}
  ...
  if(EP & 1)
  {
    ...
    while(pEP->AvbBuff)
    {
      ...
      while (Count)
      {
        ...
        if (--Count)
        {
          Data |= (uint32_t)(*(pEP->pBuffer+Offset++))<<8;
          --Count;
        }
        *pDst++ = Data;
      }

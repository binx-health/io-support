...
using AtlasGenetics.ClassA.IOReader.SignalDatabase;
using AtlasGenetics.ClassA.IOReader.TestHarness;
using AtlasGenetics.ClassA.Math;
...
namespace AtlasGenetics.MOB_VTP_027
{
    public class LogicTest : ITest
    {
        ...
        public void Execute(IOReaderTestHarness harness)
        {
            ...
            if (result)
            {
                result = harness.Assert(Assertion.Equal(new string[0], "Login to reader", harness.Login("admin", "", true), true)) &&
                         harness.Assert(Assertion.Equal(new string[0], "Set Data Policy", harness.SetDataPolicy(FieldPolicy.Ignore), true)) &&
        ...
        private static void CheckResult(IOReaderTestHarness harness, IEnumerable<string> traceablility, AssayRunResult.DiseaseStatus ctResult, AssayRunResult.DiseaseStatus ngResult)
        {
            ...
            {
                expectedResultScreen = ReaderVM.Screens.TestResult.NotDetectedDetectedBitmap;
            }
            else if (ctResult == AssayRunResult.DiseaseStatus.Detected && ngResult == AssayRunResult.DiseaseStatus.NotDetected)
            {
                expectedResultScreen = ReaderVM.Screens.TestResult.DetectedNotDetectedBitmap;
            }
            ...
            harness.Assert(Assertion.Equal(traceablility, "Disease results as expected", harness.Reader.WaitForScreen(expectedResultScreen, TimeSpan.FromSeconds(0), ReaderVM.Screens.TestResult.HeaderFooterMask, ReaderVM.Screens.TestResult.InputLine1Mask), true));
        }
    }
}
